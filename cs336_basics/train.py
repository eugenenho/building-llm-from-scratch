...
def cross_entropy(logits: Float[Tensor, "... vocab_size"], targets: Int[Tensor, "..."]):
    ...
    uber_batch_size = logits.shape[0]                                   # logits = (uber_batch_size, vocab_size)
    
    # flatten to 1D
    targets = targets.reshape(-1) # flatten to a 1D tensor              # (uber_batch_size,)
    
    # numerical stability
    max_values = torch.max(logits, dim = -1, keepdim = True).values     # (uber_batch_size, 1)
    ...
    log_z = max_values.squeeze(-1) + second_term                        # (uber_batch_size,)
