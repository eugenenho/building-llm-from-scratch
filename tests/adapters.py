...
def run_embedding(
    ...
    embed_layer.load_state_dict({"embed": weights})
...
def run_multihead_self_attention(
    ...
    msa.load_state_dict({"w_q": q_proj_weight, "w_k": k_proj_weight, "w_v": v_proj_weight, "w_o": o_proj_weight})
...
def run_transformer_lm(
    ...
    state = {
        "token_embedding.embed": weights["token_embeddings.weight"],
        "final_norm.g": weights["ln_final.weight"],
        "lm_head.w": weights["lm_head.weight"],
    }
    for i in range(num_layers):
        state[f"blocks.{i}.mha.w_q"] = weights[f"layers.{i}.attn.q_proj.weight"]
        state[f"blocks.{i}.mha.w_k"] = weights[f"layers.{i}.attn.k_proj.weight"]
        state[f"blocks.{i}.mha.w_v"] = weights[f"layers.{i}.attn.v_proj.weight"]
        state[f"blocks.{i}.mha.w_o"] = weights[f"layers.{i}.attn.output_proj.weight"]
        ...
        state[f"blocks.{i}.rmsnorm1.g"] = weights[f"layers.{i}.ln1.weight"]
        state[f"blocks.{i}.rmsnorm2.g"] = weights[f"layers.{i}.ln2.weight"]
...
def run_get_batch(
    ...
    """
    from cs336_basics.train import data_loader
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.train import learning_rate_scheduler
    return learning_rate_scheduler(t = it, max_learning_rate= max_learning_rate, min_learning_rate=min_learning_rate, warmup_iters=warmup_iters, cosine_cycle_iters=cosine_cycle_iters)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    from cs336_basics.train_bpe import train_bpe_function
    return train_bpe_function(
